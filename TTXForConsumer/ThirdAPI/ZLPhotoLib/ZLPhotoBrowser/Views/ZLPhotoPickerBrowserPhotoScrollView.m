...
- (void)setPhoto:(ZLPhotoPickerBrowserPhoto *)photo{
    ...
    if (photo.photoURL.absoluteString.length) {
        ...
        if (photoRange.location != NSNotFound){
            ...
            // 网络URL
            [_photoImageView sd_setImageWithURL:photo.photoURL placeholderImage:LoadingErrorImage options:SDWebImageRetryFailed progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                _photoImageView.progress = (double)receivedSize / expectedSize;
            } completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                if (image) {
                    _photoImageView.image = image;
                }else{
                    _photoImageView.image = LoadingErrorImage;
                }
                [weakSelf displayImage];
            }];
        }
